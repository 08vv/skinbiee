...
# 1. New renderSkinResults function
new_render_skin_results = r"""function renderSkinResults(results, imgUrl) {
...
# 2. New renderSkinProductRecommendations function
new_render_recs = r"""function renderSkinProductRecommendations(results) {
...
# 3. New renderProdResults function
new_render_prod = r"""function renderProdResults(data) {
    ...
    const title = byId('prod-verdict-title', 'prod-result-title');
    const scoreBadge = byId('prod-score-badge', 'prod-score-badge');
    const desc = byId('prod-verdict-subtitle', 'prod-result-desc');
    const ingredients = byId('prod-ingredients-text', 'prod-ingredients-text');
...
# Replacement pattern for skin results
pattern_skin = re.compile(r'function renderSkinResults.*?function renderSkinProductRecommendations.*?\}', re.DOTALL)
content = pattern_skin.sub(new_render_skin_results + "\n\n" + new_render_recs, content)

# Replacement pattern for prod results
pattern_prod = re.compile(r'function renderProdResults.*?\}', re.DOTALL)
content = pattern_prod.sub(new_render_prod, content)

with open('frontend/skinbiee.js', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated skinbiee.js successfully.")
